  ...
  return (
    <DashboardLayout role="admin">
      ...
      <div className="mb-6 flex gap-2">
        {['', 'pending', 'approved', 'rejected', 'suspended'].map((s) => (
          <button
            ...
            className={`rounded-full px-3 py-1.5 text-xs capitalize ${filter === s ? 'bg-accent-500 text-white' : 'bg-white/[0.05] text-white/50'}`}
          >
            {s || 'All'}
          </button>
        ))}
      ...
      {loading ? (
        ...
        <div className="card py-16 text-center text-white/40">No employers found for this filter.</div>
      ) : (
          ...
          {companies.map((c) => (
            <div key={c.id} className="card flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <div className="text-xs text-white/40">{c.users?.email} · {c.industry || 'Industry not set'}</div>
              </div>
              <div className="flex items-center gap-2">
                <span className={`rounded-full px-3 py-1 text-xs ${statusStyle[c.approval_status]}`}>{c.approval_status}</span>
                {c.approval_status !== 'approved' && (
                  <button onClick={() => setStatus(c.id, 'approved')} className="btn-primary !py-1.5 !px-3 text-xs">Approve</button>
                )}
                {c.approval_status !== 'rejected' && c.approval_status !== 'suspended' && (
                  <button onClick={() => setStatus(c.id, 'rejected')} className="btn-secondary !py-1.5 !px-3 text-xs">Reject</button>
                )}
                {c.approval_status === 'approved' && (
                  <button onClick={() => setStatus(c.id, 'suspended')} className="text-xs text-red-400 hover:text-red-300">Suspend</button>
                )}
              </div>
            </div>
          ))}
