...
export default function AdminCMS() {
  ...
  return (
    <DashboardLayout role="admin">
      ...
      <p className="mb-6 text-white/50">Manage homepage content: testimonials, FAQs, blog posts, and inbound queries.</p>

      <div className="mb-6 flex flex-wrap gap-2">
        {TABS.map((t) => (
          <button
            key={t} onClick={() => setTab(t)}
            className={`rounded-full px-3 py-1.5 text-xs ${tab === t ? 'bg-accent-500 text-white' : 'bg-white/[0.05] text-white/50'}`}
          >
            {t}
          </button>
        ))}
...
function TestimonialsPanel() {
  ...
  const add = async (e) => {
    ...
    const { error } = await supabase.from('testimonials').insert({ ...form, is_featured: true });
    if (error) { toast.error(error.message); return; }
    setForm({ name: '', role: '', company: '', quote: '', rating: 5 });
    load();
  };

  const toggleFeature = async (id, val) => {
      ...
      <div className="space-y-2">
        {items.map((t) => (
          <div key={t.id} className="card flex items-center justify-between">
            <div>
              <div className="text-sm font-medium">{t.name} — {t.role}, {t.company}</div>
              <div className="text-xs text-white/50">"{t.quote}"</div>
            </div>
            <div className="flex items-center gap-2">
              <button onClick={() => toggleFeature(t.id, t.is_featured)} className="text-white/40 hover:text-white">
                {t.is_featured ? <Eye size={16} /> : <EyeOff size={16} />}
              </button>
...
function FaqsPanel() {
  ...
  const add = async (e) => {
    ...
    await supabase.from('faqs').insert({ ...form, sort_order: items.length + 1 });
        ...
        {items.map((f) => (
          <div key={f.id} className="card flex items-start justify-between">
            <div>
              ...
              <div className="text-xs text-white/50">{f.answer}</div>
            </div>
            <button onClick={() => remove(f.id)} className="shrink-0 text-white/40 hover:text-red-400"><Trash2 size={16} /></button>
          </div>
        ))}
      </div>
    </div>
...
function BlogsPanel() {
  ...
  const add = async (e) => {
    ...
    const { error } = await supabase.from('blogs').insert({ ...form, slug, is_published: false });
    if (error) { toast.error(error.message); return; }
  ...
  const togglePublish = async (id, val) => {
    await supabase.from('blogs').update({ is_published: !val, published_at: !val ? new Date().toISOString() : null }).eq('id', id);
    load();
  };
        ...
        {items.map((b) => (
          <div key={b.id} className="card flex items-center justify-between">
            <div>
              ...
              <div className="text-xs text-white/50">{b.excerpt}</div>
            </div>
            <div className="flex items-center gap-3">
              <span className={`rounded-full px-2 py-1 text-xs ${b.is_published ? 'bg-green-500/15 text-green-300' : 'bg-white/10 text-white/50'}`}>
                {b.is_published ? 'Published' : 'Draft'}
              </span>
              <button onClick={() => togglePublish(b.id, b.is_published)} className="text-xs text-accent-400 hover:text-accent-300">
...
function ContactPanel() {
  ...
  return (
    <div className="space-y-2">
      {items.length === 0 ? (
        <div className="card py-12 text-center text-white/40">No contact queries yet.</div>
      ) : items.map((c) => (
        <div key={c.id} className="card flex items-start justify-between">
          <div>
            <div className="text-sm font-medium">{c.name} — {c.email}</div>
            <div className="text-xs text-white/50">{c.subject}</div>
            <div className="mt-1 text-xs text-white/40">{c.message}</div>
          </div>
          {!c.is_resolved ? (
            <button onClick={() => resolve(c.id)} className="btn-secondary shrink-0 !py-1.5 !px-3 text-xs">Mark resolved</button>
          ) : (
            <span className="shrink-0 rounded-full bg-green-500/15 px-2 py-1 text-xs text-green-300">Resolved</span>
          )}
        </div>
      ))}
    </div>
...
function NewsletterPanel() {
  const [items, setItems] = useState([]);
  useEffect(() => {
    supabase.from('newsletter').select('*').order('subscribed_at', { ascending: false }).then(({ data }) => setItems(data || []));
  }, []);
        ...
        <tbody className="divide-y divide-white/[0.06]">
          {items.map((n) => (
            <tr key={n.id}>
              ...
              <td className="px-4 py-3 text-white/40">{new Date(n.subscribed_at).toLocaleDateString()}</td>
            </tr>
          ))}
        </tbody>
      </table>
