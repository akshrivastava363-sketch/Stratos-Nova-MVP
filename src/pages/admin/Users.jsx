...
export default function AdminUsers() {
  ...
  return (
    <DashboardLayout role="admin">
      ...
      <p className="mb-6 text-white/50">Every candidate, employer, and admin on the platform.</p>
      ...
      <div className="card mb-6 flex flex-wrap gap-3">
        <div className="flex flex-1 items-center gap-2 rounded-xl bg-white/[0.04] px-3 py-2">
          ...
          <input className="w-full bg-transparent text-sm outline-none" placeholder="Search by email" value={query} onChange={(e) => setQuery(e.target.value)} />
        </div>
      ...
      <div className="card !p-0">
        {loading ? (
          <div className="p-6"><div className="skeleton h-40" /></div>
        ) : (
              ...
              {users.map((u) => (
                <tr key={u.id}>
                  ...
                  <td className="px-4 py-3">
                    <span className={`rounded-full px-2 py-1 text-xs ${u.is_active ? 'bg-green-500/15 text-green-300' : 'bg-red-500/15 text-red-300'}`}>
                      {u.is_active ? 'Active' : 'Suspended'}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-white/40">{new Date(u.created_at).toLocaleDateString()}</td>
                  <td className="px-4 py-3">
                    <button onClick={() => toggleActive(u.id, u.is_active)} className="text-xs text-accent-400 hover:text-accent-300">
