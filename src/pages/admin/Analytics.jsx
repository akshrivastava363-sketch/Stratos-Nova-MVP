  ...
  useEffect(() => {
    (async () => {
      ...
      (apps || []).forEach((a) => {
        ...
        if (title) jobCounts[title] = (jobCounts[title] || 0) + 1;
      });
  ...
  return (
    <DashboardLayout role="admin">
      ...
      <div className="mb-8 grid grid-cols-2 gap-4 sm:grid-cols-5">
        {stats && [
          ['Total Users', stats.totalUsers], ['Candidates', stats.candidates], ['Employers', stats.employers],
          ['Jobs', stats.jobs], ['Applications', stats.applications],
        ].map(([label, val]) => (
      ...
      <div className="mb-6 grid gap-6 lg:grid-cols-2">
        <div className="card">
          <h2 className="mb-4 text-lg font-semibold">Monthly Growth (new users)</h2>
          {loading ? <div className="skeleton h-40" /> : growth.length === 0 ? (
              ...
              {growth.map(([label, count]) => {
                ...
                return (
                  <div key={label} className="flex flex-1 flex-col items-center gap-2">
                    <div className="w-full rounded-t-lg bg-accent-500" style={{ height: `${(count / max) * 100}%`, minHeight: 4 }} />
