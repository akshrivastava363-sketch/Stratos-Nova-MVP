...
import { MapPin, Briefcase, IndianRupee, Bookmark, CheckCircle2 } from 'lucide-react';
...
export default function JobDetails() {
  ...
  useEffect(() => {
    (async () => {
      const { data } = await supabase.from('jobs').select('*,companies(name,logo_url,about)').eq('id', id).single();
      setJob(data);
  ...
  const toggleSave = async () => {
    ...
    if (saved) { await supabase.from('saved_jobs').delete().eq('job_id', id).eq('candidate_id', user.id); setSaved(false); }
    else { await supabase.from('saved_jobs').insert({ job_id: id, candidate_id: user.id }); setSaved(true); toast.success('Saved'); }
  };
  ...
  const handleApply = async () => {
    ...
    const { error } = await supabase.from('applications').insert({ job_id: id, candidate_id: user.id, cover_note: coverNote, resume_url: profile?.resume_url || null });
    setApplying(false);
    if (error) { toast.error(error.message); return; }
    setApplied(true); setShowApplyForm(false); toast.success('Application submitted!');
  };

  if (!job) return <div className="min-h-screen bg-nova-950"><Navbar /><div className="mx-auto max-w-3xl px-6 py-10"><div className="skeleton h-96" /></div></div>;
        ...
        <div className="card mt-4">
          ...
          {skills.length > 0 && (
            <div className="mt-4 flex flex-wrap gap-2">
              {skills.map((s) => <span key={s} className="badge bg-white/5 text-white/60">{s}</span>)}
            </div>
          )}
          <div className="mt-6 border-t border-white/[0.06] pt-6">
            {applied ? (
              <div className="flex items-center gap-2 rounded-xl bg-green-500/10 px-4 py-3 text-sm text-green-300"><CheckCircle2 size={18} /> You've applied to this job.</div>
            ) : showApplyForm ? (
              <div className="space-y-3">
                <textarea className="input-field" rows={3} placeholder="Note to employer (optional)" value={coverNote} onChange={(e) => setCoverNote(e.target.value)} />
              ...
              <button onClick={() => (user ? setShowApplyForm(true) : navigate('/login'))} className="btn-primary w-full">Apply Now</button>
            )}
          </div>
        </div>
        <div className="card mt-4">
