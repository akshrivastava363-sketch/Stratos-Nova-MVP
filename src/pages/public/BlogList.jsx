  ...
  return (
    <div className="min-h-screen bg-nova-950">
      ...
      <div className="mx-auto max-w-4xl px-6 py-10">
        ...
        <p className="mb-8 text-white/50">Hiring tips and product updates for startup founders.</p>
          ...
          <div className="space-y-4">
            {posts.map((p) => (
              <Link to={`/blog/${p.slug}`} key={p.id} className="card block">
                {p.cover_image_url && <img src={p.cover_image_url} className="mb-3 h-40 w-full rounded-xl object-cover" />}
                <h2 className="text-lg font-semibold">{p.title}</h2>
                <p className="mt-1 text-sm text-white/50">{p.excerpt}</p>
                <div className="mt-2 text-xs text-white/30">{new Date(p.published_at).toLocaleDateString()}</div>
              </Link>
            ))}
          </div>
        )}
