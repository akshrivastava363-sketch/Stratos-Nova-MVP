...
export default function CandidateApplications() {
  ...
  const load = async () => {
    const { data } = await supabase
      ...
      .eq('candidate_id', user.id).order('applied_at', { ascending: false });
    ...
    <DashboardLayout role="candidate">
      ...
      {loading ? (
        ...
        <div className="card py-16 text-center text-white/40">
          ...
          No applications yet. <Link to="/jobs" className="text-accent-400 hover:text-accent-300">Browse jobs →</Link>
        </div>
      ) : (
          ...
          {applications.map((a) => (
            <div key={a.id} className="card flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <div className="text-xs text-white/30">Applied {new Date(a.applied_at).toLocaleDateString()}</div>
              </div>
              <div className="flex items-center gap-3">
                <span className={`rounded-full px-3 py-1 text-xs ${statusColor[a.status] || 'bg-white/10'}`}>{a.status?.replace(/_/g, ' ')}</span>
                {!['withdrawn', 'rejected', 'joined'].includes(a.status) && (
                  <button onClick={() => withdraw(a.id)} className="text-xs text-white/40 hover:text-red-400">Withdraw</button>
                )}
              </div>
            </div>
          ))}
