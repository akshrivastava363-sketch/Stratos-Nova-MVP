...
export default function SavedJobs() {
  ...
  const load = async () => {
    const { data } = await supabase
      .from('saved_jobs')
      .select('job_id,saved_at,jobs(id,title,location,work_mode,employment_type,companies(name))')
      .eq('candidate_id', user.id).order('saved_at', { ascending: false });
    setSaved(data || []);
    setLoading(false);
  };
  ...
  return (
    <DashboardLayout role="candidate">
      ...
      {loading ? (
        <div className="grid gap-4 sm:grid-cols-2">{[...Array(4)].map((_, i) => <div key={i} className="skeleton h-24" />)}</div>
      ) : saved.length === 0 ? (
        <div className="card py-16 text-center text-white/40">
          <Bookmark size={32} className="mx-auto mb-3 opacity-30" />
          No saved jobs yet. <Link to="/jobs" className="text-accent-400 hover:text-accent-300">Browse jobs →</Link>
        </div>
      ) : (
          ...
          {saved.map((s) => (
            <div key={s.job_id} className="card">
              ...
              <div className="mt-3 flex items-center justify-between">
                <span className="rounded-full bg-white/5 px-2 py-1 text-xs text-white/50">{s.jobs?.work_mode}</span>
