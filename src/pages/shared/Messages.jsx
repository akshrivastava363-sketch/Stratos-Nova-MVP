    ...
    <DashboardLayout role={role}>
      ...
      <div className="card grid h-[65vh] grid-cols-[220px_1fr] gap-0 overflow-hidden !p-0">
        <div className="overflow-y-auto border-r border-white/[0.06] p-3">
          {threads.length === 0 && !presetTo ? (
            <p className="p-3 text-xs text-white/40">No conversations yet.</p>
          ) : (
            <>
              {presetTo && !threads.find((t) => t.userId === presetTo) && (
                <button
                  onClick={() => setActiveUserId(presetTo)}
                  className={`mb-1 w-full rounded-lg px-3 py-2.5 text-left text-sm ${activeUserId === presetTo ? 'bg-accent-500/15 text-accent-300' : 'hover:bg-white/[0.05]'}`}
                >
                  New conversation
                </button>
              )}
              {threads.map((t) => (
                <button
                  key={t.userId} onClick={() => setActiveUserId(t.userId)}
                  className={`mb-1 w-full rounded-lg px-3 py-2.5 text-left text-sm ${activeUserId === t.userId ? 'bg-accent-500/15 text-accent-300' : 'hover:bg-white/[0.05]'}`}
                >
                  <div className="truncate">{t.lastMessage}</div>
                  <div className="text-[10px] text-white/30">{new Date(t.at).toLocaleDateString()}</div>
                </button>
              ))}
            </>
          )}
        ...
        <div className="flex flex-col">
          <div className="flex-1 space-y-3 overflow-y-auto p-4">
            {!activeUserId ? (
              <p className="text-center text-sm text-white/30">Select a conversation</p>
            ) : messages.length === 0 ? (
              <p className="text-center text-sm text-white/30">Say hello 👋</p>
            ) : (
              messages.map((m) => (
                <div key={m.id} className={`flex ${m.sender_id === user.id ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[70%] rounded-2xl px-4 py-2 text-sm ${m.sender_id === user.id ? 'bg-accent-500 text-white' : 'bg-white/[0.06] text-white/80'}`}>
                    {m.body}
                  </div>
                </div>
              ))
