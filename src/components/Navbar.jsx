...
export default function Navbar() {
  ...
  const dashboardPath = role === 'employer' ? '/employer/dashboard'
    ...
    <header className="sticky top-0 z-50 border-b border-white/[0.06] bg-nova-950/80 backdrop-blur-xl">
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
        ...
        <div className="hidden items-center gap-8 md:flex">
          {links.map((l) => (
            <a key={l.label} href={l.href} className="text-sm text-white/70 transition hover:text-white">
              {l.label}
            </a>
          ))}
        </div>
        ...
        <div className="hidden items-center gap-3 md:flex">
          {user ? (
            <>
              <button onClick={() => navigate(dashboardPath)} className="btn-secondary !py-2 !px-4 text-sm">
                Dashboard
              </button>
      ...
      {open && (
        <div className="border-t border-white/[0.06] px-6 py-4 md:hidden">
          <div className="flex flex-col gap-4">
            {links.map((l) => (
              <a key={l.label} href={l.href} className="text-sm text-white/70">{l.label}</a>
            ))}
